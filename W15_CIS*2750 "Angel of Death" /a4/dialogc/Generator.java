...
public class Generator {
    ...
    private String illegalFValue;
    private Settings settings;

    public Generator(Settings settings) {
        this.settings = settings;
        illegalFValue = "public class IllegalFieldValueException extends Exception {" + "\n" +
    ...
    public void generate() {
        ...
        if(pmp.isSuccess()) {
            ...
            PrintWriter illegalFValueWriter;
            log.append("Attempting to write IllegalFieldValueException.java...\n");
            try {
                illegalFValueWriter = new PrintWriter(compileDir + "/IllegalFieldValueException.java", "UTF-8");
                illegalFValueWriter.printf("%s\n", illegalFValue);
                illegalFValueWriter.close();
            } catch (IOException e) {
                log.append("ERROR: Something went wrong writing IllegalFieldValueException.java\n");
            }
    ...
    */
    public void generateSource(PM_Parser data) {
        ...
        for(int i = 0; i < data.getFieldName().length; i++) {
            if(data.getFieldValue()[i].equals("string")) {
                sb.append("\t" + "public String getDC" + data.getFieldName()[i] + "() throws IllegalFieldValueException {" + "\n" +
                        "\t\t" + "return " + data.getFieldName()[i] + ".getText();" + "\n" +
                          "\t" + "}" + "\n\n"
                );
            } else {
                sb.append("\t" + "public String getDC" + data.getFieldName()[i] + "() throws IllegalFieldValueException {" + "\n" +
                        "\t\t" + "if(" + data.getFieldValue()[i] + "Check(" + data.getFieldName()[i] + ".getText())) {" + "\n" +
                        "\t\t\t" + "return " + data.getFieldName()[i] + ".getText();" + "\n" +
                        "\t\t" + "} else {" + "\n" +
                        "\t\t\t" + "throw new IllegalFieldValueException(" + data.getFieldName()[i] + ".getText());" + "\n" +
                        "\t\t" + "}" + "\n" +
                        "\t" + "}" + "\n\n"
                );
            }
            
            sb.append("\t" + "public void setDC" + data.getFieldName()[i] + "(String str) {" + "\n" +
                    "\t\t" + data.getFieldName()[i] + ".setText(str);" + "\n" +
                      "\t" + "}" + "\n\n"
            );
        }
    ...
    */
    public void generateFieldEdit(PM_Parser data) {
        ...
        for(String s: data.getFieldName()) {
            sb.append("\t" + "String getDC" + s + "() throws IllegalFieldValueException;" + "\n\n" +
