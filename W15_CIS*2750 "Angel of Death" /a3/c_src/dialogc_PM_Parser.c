/* filename:   dialogc_PM_Parser.c
 * student:    Kyle Hersey
 * student#:   0851973
 * course:     CIS*2750
 * assignment: 3
 * 
 * This file contains JNI functions for interfacing between Java and libpm
 */
...
JNIEXPORT jobjectArray JNICALL Java_dialogc_PM_1Parser_parseValues(JNIEnv *env, jobject object, jstring jfilename, jobjectArray jfields) {
	...
	for(i = 0; i < list_length; i++) {
		...
		#ifdef _DEBUG_
		printf("parseValues: registering %s\n",current);
		#endif
	...
	for(i = 0; i < list_length; i++) {
		...
		value = PM_getValue(param, current).str_val;
		#ifdef _DEBUG_
		printf("parseValues: for current=%s value=%s \n",current,value);
		#endif
